namespace MightyTerrainMesh
{
    ...
    public class CreateMeshJob
    {
        ...
        public CreateMeshJob(Terrain t, Bounds VolumnBound, int mx, int mz, List<MeshLODCreate> setting, bool createBoundry):this(t, VolumnBound,mx, mz, setting.ToArray(), createBoundry)
        {
        }

        public CreateMeshJob(Terrain t, Bounds VolumnBound, int mx, int mz, MeshLODCreate[] setting, bool createBoundry)
        {
            ...
            for (int i = 0; i < setting.Length; ++i)
            {
                ...
                {
                    LODs[i] = new MTTerrainScanner(t, VolumnBound, s.Subdivision, s.SlopeAngleError, mx, mz, false);
                }
        ...
        public void EndProcess(ref MTTerrainScanner customDetail)
        {
            int startIndex = customDetail == null ? 1 : 0;

            if (customDetail == null)
            {
                customDetail = LODs[0];
                customDetail.FillData();
                for (int i = startIndex; i < LODs.Length; ++i)
                {
                    MTTerrainScanner scaner = LODs[i];
                    for (int t = 0; t < customDetail.Trees.Length; ++t)
                    {
                        SamplerTree dt = customDetail.Trees[t];
            ...
            {
                //这里要求scanner一定是customDetail更coarse的一级
                int sizeDetail = customDetail.maxZ;

                for (int i = startIndex; i < LODs.Length; ++i)
                {
                    MTTerrainScanner scaner = LODs[i];
                    int sizeScanner = scaner.maxZ;

                    for (int t = 0; t < customDetail.Trees.Length; ++t)
                    {
                        int detailX = t / sizeDetail;
                        int detailY = t % sizeDetail;

                        int scannerX = detailX / 2;
                        int scannerY = detailY / 2;

                        SamplerTree dt = customDetail.Trees[t];
                        SamplerTree lt = scaner.Trees[scannerX * sizeScanner + scannerY];

                        Vector3 offset = dt.Center - lt.Center;
                        List<SampleVertexData> topVertecies = new List<SampleVertexData>();
                        List<SampleVertexData> bottomVertecies = new List<SampleVertexData>();
                        List<SampleVertexData> leftVertecies = new List<SampleVertexData>();
                        List<SampleVertexData> rightVertecies = new List<SampleVertexData>();
                        if(offset.x < 0)
                        {
                            if(offset.z < 0)
                            {
                                lt.Boundaries.Add(SamplerTree.LBCorner, dt.Boundaries[SamplerTree.LBCorner]);
                                bottomVertecies.AddRange(dt.Boundaries[SamplerTree.BBorder]);
                                bottomVertecies.AddRange(dt.Boundaries[SamplerTree.RBCorner]);
                                leftVertecies.AddRange(dt.Boundaries[SamplerTree.LBorder]);
                                leftVertecies.AddRange(dt.Boundaries[SamplerTree.LTCorner]);
                            }
                            else
                            {
                                lt.Boundaries.Add(SamplerTree.LTCorner, dt.Boundaries[SamplerTree.LTCorner]);
                                topVertecies.AddRange(dt.Boundaries[SamplerTree.TBorder]);
                                topVertecies.AddRange(dt.Boundaries[SamplerTree.RTCorner]);
                                leftVertecies.AddRange(dt.Boundaries[SamplerTree.LBorder]);

                                //避免重复
                                //leftVertecies.AddRange(dt.Boundaries[SamplerTree.LBCorner]);
                            }
                        }
                        else
                        {
                            if (offset.z < 0)
                            {
                                lt.Boundaries.Add(SamplerTree.RBCorner, dt.Boundaries[SamplerTree.RBCorner]);
                                bottomVertecies.AddRange(dt.Boundaries[SamplerTree.BBorder]);
                                rightVertecies.AddRange(dt.Boundaries[SamplerTree.RBorder]);
                                rightVertecies.AddRange(dt.Boundaries[SamplerTree.RTCorner]);

                                //避免重复
                                //bottomVertecies.AddRange(dt.Boundaries[SamplerTree.LBCorner]);
                            }
                            else
                            {
                                lt.Boundaries.Add(SamplerTree.RTCorner, dt.Boundaries[SamplerTree.RTCorner]);
                                topVertecies.AddRange(dt.Boundaries[SamplerTree.TBorder]);
                                rightVertecies.AddRange(dt.Boundaries[SamplerTree.RBorder]);
                                
                                
                                //避免重复
                                //topVertecies.AddRange(dt.Boundaries[SamplerTree.LTCorner]);
                                //rightVertecies.AddRange(dt.Boundaries[SamplerTree.RBCorner]);
                            }
                        }

                        if (!lt.Boundaries.ContainsKey(SamplerTree.BBorder))
                        {
                            lt.Boundaries.Add(SamplerTree.BBorder, new List<SampleVertexData>());
                        }
                        if (!lt.Boundaries.ContainsKey(SamplerTree.LBorder))
                        {
                            lt.Boundaries.Add(SamplerTree.LBorder, new List<SampleVertexData>());
                        }
                        if (!lt.Boundaries.ContainsKey(SamplerTree.TBorder))
                        {
                            lt.Boundaries.Add(SamplerTree.TBorder, new List<SampleVertexData>());
                        }
                        if (!lt.Boundaries.ContainsKey(SamplerTree.RBorder))
                        {
                            lt.Boundaries.Add(SamplerTree.RBorder, new List<SampleVertexData>());
                        }
                        lt.Boundaries[SamplerTree.BBorder].AddRange(bottomVertecies);
                        lt.Boundaries[SamplerTree.LBorder].AddRange(leftVertecies);
                        lt.Boundaries[SamplerTree.TBorder].AddRange(topVertecies);
                        lt.Boundaries[SamplerTree.RBorder].AddRange(rightVertecies);
                    }
                    scaner.FillData();
                }
    ...
    public class CreateDataJob
    {
        ...
        //public CreateDataJob(Terrain t, Bounds VolumnBound, int depth, MeshLODCreate[] setting, float minEdge, bool createBoundry)
        //{
        //    LODs = new MTTerrainScanner[setting.Length];
        ...
        //        else
        //        {
        //            LODs[i] = new MTTerrainScanner(t, VolumnBound, subdiv, angleErr, grid_count, grid_count, false);
        //        }
        //    }
        //}
        //public void Update()
