namespace MightyTerrainMesh
{
    ...
    public abstract class SamplerBase
    {
        ...
        protected float accumulatedAngleErr;
    }
    public class SamplerLeaf : SamplerBase
    {
        ...
        public SamplerLeaf(Vector3 center, Vector2 uv, float accumulatedAngleErr)
        {
            mVertex = new SampleVertexData();
            mVertex.Position = center;
            mVertex.UV = uv;
            this.accumulatedAngleErr = accumulatedAngleErr;
        }

        public SamplerLeaf(SampleVertexData vert, float accumulatedAngleErr)
        {
            mVertex = vert;
            this.accumulatedAngleErr = accumulatedAngleErr;
        }
    ...
    public class SamplerNode : SamplerBase
    {
        ...
        public SamplerNode(int sub, Vector3 center, Vector2 size, Vector2 uv, Vector2 uvstep)
        {
            ...
            else
            {
                Children[0] = new SamplerLeaf(new Vector3(center.x - 0.5f * subsize.x, center.y, center.z - 0.5f * subsize.y),
                    new Vector2(uv.x - 0.5f * subuvstep.x, uv.y - 0.5f * subuvstep.y), 0f);
        ...
        public SamplerLeaf Combine(float angleErr)
        {
            ...
            for (int i = 0; i < Children.Length; ++i)
            {
                ...
                float lAngleErr = Mathf.Abs(Mathf.Rad2Deg * Mathf.Acos(dot));
                accumulatedAngleErr += lAngleErr;
                if (accumulatedAngleErr >= angleErr)
                    return null;
            }

            SamplerLeaf leaf = new SamplerLeaf(mVertex, accumulatedAngleErr * 2f); //加强轮廓变化强烈处的误差贡献
            for (int i = 0; i < Children.Length; ++i)
            {
                SamplerLeaf l = (SamplerLeaf)Children[i];
