Shader "TerrainSystem/HRP/TerrainLitAtlasSplatmap" 
{
	Properties{
		[NoScaleOffset]_Control("Control (RGBB)", 2D) = "blue" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half mipmapLevel(half2 uv, half2 textureSize)
			{
				half dx = ddx(uv * textureSize.x).x;
				half dy = ddy(uv * textureSize.y).x;
				half d = max(dot(dx, dx), dot(dy, dy));
				return min(7.0h, 0.5h * log2(d));//0.5是技巧，本来是d的平方。
			}

			void SplatmapMix(in float4 uvSplat01, inout half2 splatControl, out half4 mixedDiffuse, out half2 defaultSmoothness, inout half3 mixedNormal)
			{
				half4 diffAlbedo[2];
				...
				defaultSmoothness = half2(1.0h, 1.0h) - half2(diffAlbedo[0].a, diffAlbedo[1].a);
			...
			#ifdef _NORMALMAP
				half3 nrm = 0.0f;
				nrm += splatControl.r * UnpackNormalmapRGorAG(SAMPLE_TEXTURE2D_BIAS(_Normal0, sampler_Control, uvSplat01.xy, _ScaleFactor.y));
				nrm += splatControl.g * UnpackNormalmapRGorAG(SAMPLE_TEXTURE2D_BIAS(_Normal0, sampler_Control, uvSplat01.zw, _ScaleFactor.y));
			...
			half4 SplatmapFragment(Varyings IN) : SV_TARGET
			{
				...
				half2 defaultSmoothness = 0.0h;
				float2 splatUV = (IN.uvMainAndLM.xy * (_Control_TexelSize.zw - 1.0f) + 0.5f) * _Control_TexelSize.xy;
				
				uint4 controlVec = SAMPLE_TEXTURE2D(_Control, sampler_Control, splatUV) * 15.0;

				half control = (controlVec.b + (controlVec.a << 4)) / 255.0h;
				half2 splatControl = half2(control, 1 - control);
				...
				SplatmapMix(uvSplat01, splatControl, mixedDiffuse, defaultSmoothness, normalTS);
				half3 albedo = mixedDiffuse.rgb;

				half2 defaultMetallic = half2(/*_Metallic0*/ 0.0h, /*_Metallic1*/ 0.0h);
				half2 defaultOcclusion = 1.0h;
