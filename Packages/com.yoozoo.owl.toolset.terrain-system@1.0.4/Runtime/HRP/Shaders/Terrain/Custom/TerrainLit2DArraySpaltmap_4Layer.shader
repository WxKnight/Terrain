Shader "TerrainSystem/HRP/TerrainLit2DArraySplatmap_4Layer" 
{
	Properties{
		[NoScaleOffset]_Control("Control", 2D) = "blue" {}
		[NoScaleOffset]_Weight("", 2D) = "white" {}
		[NoScaleOffset]_Splat0("DiffuseAtlas (Albedo: RGB, Roughness: A)", 2DArray) = "white" {}
		[NoScaleOffset]_Normal0("NormalAtlas", 2DArray) = "bump" {}
...
#define _NORMALMAP
			...
			TEXTURE2D_ARRAY(_Splat0); SAMPLER(sampler_Splat0);
			#ifdef _NORMALMAP
			TEXTURE2D_ARRAY(_Normal0);	
			#endif
			...
			void SplatmapMix(in half4 uvSplat01, in half4 uvSplat23, in half4 index, inout half4 splatControl, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
			{
				half4 diffAlbedo[4];

				diffAlbedo[0] = SAMPLE_TEXTURE2D_ARRAY(_Splat0, sampler_Splat0, uvSplat01.xy, index.r);
				diffAlbedo[1] = SAMPLE_TEXTURE2D_ARRAY(_Splat0, sampler_Splat0, uvSplat01.zw, index.g);
				diffAlbedo[2] = SAMPLE_TEXTURE2D_ARRAY(_Splat0, sampler_Splat0, uvSplat23.xy, index.b);
				diffAlbedo[3] = SAMPLE_TEXTURE2D_ARRAY(_Splat0, sampler_Splat0, uvSplat23.zw, index.a);
			...
			#ifdef _NORMALMAP
				half3 nrm = 0.0f;
				nrm += splatControl.r * UnpackNormalRGBNoScale(SAMPLE_TEXTURE2D_ARRAY(_Normal0, sampler_Splat0, uvSplat01.xy, index.r)).yxz;
				nrm += splatControl.g * UnpackNormalRGBNoScale(SAMPLE_TEXTURE2D_ARRAY(_Normal0, sampler_Splat0, uvSplat01.zw, index.g)).yxz;
				nrm += splatControl.b * UnpackNormalRGBNoScale(SAMPLE_TEXTURE2D_ARRAY(_Normal0, sampler_Splat0, uvSplat23.xy, index.b)).yxz;
				nrm += splatControl.a * UnpackNormalRGBNoScale(SAMPLE_TEXTURE2D_ARRAY(_Normal0, sampler_Splat0, uvSplat23.zw, index.a)).yxz;
			...
			half4 SplatmapFragment(Varyings IN) : SV_TARGET
			{
				...
				uvSplat01.xy = splatUV * half2(_TilingScales[controlVec.x].x, _TilingScales[controlVec.x].y);
				uvSplat01.zw = splatUV * half2(_TilingScales[controlVec.y].x, _TilingScales[controlVec.y].y);
				uvSplat23.xy = splatUV * half2(_TilingScales[controlVec.z].x, _TilingScales[controlVec.z].y);
				uvSplat23.zw = splatUV * half2(_TilingScales[controlVec.w].x, _TilingScales[controlVec.w].y);

				//*****************************************************************
				float2 discreteSplatUV = (floor(splatUV * _Control_TexelSize.zw) + (frac(splatUV * _Control_TexelSize.zw) - 0.5) * 0.75 + 0.5)*  _Control_TexelSize.xy ;


				half4 splatControl = SAMPLE_TEXTURE2D(_Weight, sampler_Weight, discreteSplatUV).xyzw;

				SplatmapMix(uvSplat01, uvSplat23, controlVec, splatControl, mixedDiffuse, defaultSmoothness, normalTS);
